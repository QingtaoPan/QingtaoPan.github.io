(function () {
  ...
  var fish = document.getElementById("site-cat-fish");
  if (!cat || !fish) return;
  ...
  assetUrls.push(fish.style.getPropertyValue("--cat-fish").trim().replace(/^url\(["']?|["']?\)$/g, ""));
  ...
  function setTarget(clientX, clientY) {
    clickX = clientX;
    fishHit = false;
    fish.classList.remove("is-visible", "is-eaten");
    fish.style.left = clientX - fish.offsetWidth / 2 + "px";
    fish.style.top = clientY - fish.offsetHeight / 2 + "px";
    void fish.offsetWidth;
    fish.classList.add("is-visible");
  ...
  function tick(now) {
    if (state === "running") {
      ...
      var attackElapsed = now - attackStarted;
      if (!fishHit && attackElapsed >= attackHitAt) {
        fishHit = true;
        fish.classList.remove("is-visible");
